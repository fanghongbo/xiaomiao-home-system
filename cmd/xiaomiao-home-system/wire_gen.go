...
func wireApp(confServer *conf.Server, confData *conf.Data, config *conf.Config, static *conf.Static, registry *nacos.Registry, logger log.Logger) (*kratos.App, func(), error) {
	...
	userGroupUsecase := biz.NewUserGroupUsecase(userGroupRepo, logger)
	userGroupService := service.NewUserGroupService(userGroupUsecase, config, logger)
	...
	grpcServer := server.NewGRPCServer(confServer, userService, userGroupService, roleService, userNotificationService, logger)
	httpServer := server.NewHTTPServer(confServer, config, userService, userGroupService, roleService, userNotificationService, logger)
