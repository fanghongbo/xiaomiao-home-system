...
func (filter *Filter) Init() error {
	return filter.loadEmbeddedWordDictDir("dict")
}
...
// LoadWordDictDir 加载敏感词字典目录
func (filter *Filter) LoadWordDictDir(path string) error {
	...
	for _, file := range files {
		if file.IsDir() {
			filter.LoadWordDictDir(path + "/" + file.Name())
		} else {
			filter.LoadWordDict(path + "/" + file.Name())
		}
	}
...
// loadEmbeddedWordDictDir 加载嵌入的敏感词字典目录
func (filter *Filter) loadEmbeddedWordDictDir(dir string) error {
	files, err := fs.ReadDir(embeddedDict, dir)
	if err != nil {
		return err
	}
	...
	for _, file := range files {
		...
		if file.IsDir() {
			if err := filter.loadEmbeddedWordDictDir(filePath); err != nil {
				return err
			}
			continue
		}

		f, err := embeddedDict.Open(filePath)
		if err != nil {
			return err
		}

		if err := filter.Load(f); err != nil {
			f.Close()
			return err
		}
		f.Close()
